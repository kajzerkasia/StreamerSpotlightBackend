...
export class VoteRecord implements VoteEntity {
  ...
  async insert(): Promise<string> {
    ...
    await pool.execute(
      "INSERT INTO `votes`(`id`, `streamerId`, `upvotes`, `downvotes`) VALUES(:id, :streamerId, :upvotes, :downvotes)",
      this
    );
    return this.id;
  }

  static async getOneByStreamerId(
    streamerId: string
  ): Promise<VoteRecord | null> {
    const [results] = (await pool.execute(
      "SELECT * FROM `votes` WHERE `streamerId` = :streamerId",
      {
        streamerId,
      }
    )) as VoteRecordResults;
  ...
  async update() {
    await pool.execute(
      "UPDATE `votes` SET `upvotes` = :upvotes, `downvotes` = :downvotes WHERE `id` = :id",
      {
        id: this.id,
